  ...
  */
	static async Create(stub, params) {
		...
		try {
		  ...
		  await this.stub.invokeChaincode(EARTH_CHAINCODE_ID, ['token.create', JSON.stringify(iTokenReq)]);
		  const sTokenReq = {
		  name: createRequest.name + "itoken",
		  ...
		  await this.stub.invokeChaincode(EARTH_CHAINCODE_ID, ['token.create', JSON.stringify(sTokenReq)]);
