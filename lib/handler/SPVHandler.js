  ...
  */
	static async Create(stub, params) {
		...
		try {
		  ...
		  const user = await stub.invokeChaincode(EARTH_CHAINCODE_ID, ['token.create', JSON.stringify(iTokenReq)]);
		  logger.debug('- invokeChaincode Options:%j', user);
		  const sTokenReq = {
		  name: createRequest.name + 'itoken',
		  symbol: 's' + createRequest.name,
		  decimals: 8,
		  amount: 100000,
