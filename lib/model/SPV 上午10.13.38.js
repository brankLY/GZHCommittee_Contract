...
const logger = require('../utils/Logger').getLogger('SSA');
...
class SPV extends BaseModel {
	...
	async getOneSSA(id) {
		const method = 'getOneSSA';
		try {
			logger.enter(method);
			logger.debug(' %s- targetSSA id is %s', method, id);
			...
			if(!model){
				throw new Error('Do not have this ssa');
			}
  	...
  	async addProposalRecord(id) {
	    ...
	    try {
			...
			logger.debug(' %s- targetSSA id is %j', method, this.id);
